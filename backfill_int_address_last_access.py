...
latest_relationship AS (
    SELECT
        ...
        argMax(a.relationship, (a.block_number, g.transaction_index, a.internal_index)) AS relationship
    FROM default.canonical_execution_address_appearances a FINAL
    GLOBAL JOIN get_tx_success g
        ON lower(a.transaction_hash) = g.transaction_hash
    WHERE a.block_number BETWEEN {start_block} AND {end_block}
    AND a.block_number < 19426587
    AND a.relationship IN ('create', 'suicide')
    GROUP BY address
),
accounts_last_access AS (
    SELECT
        a.address,
...
FROM accounts_last_access ala
GLOBAL LEFT JOIN latest_relationship lr
    ON ala.address = lr.address
"""
