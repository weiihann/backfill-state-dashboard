"""Utility functions for the backfill operations."""
...
def get_min_block_from_source_tables(engine, source_tables: List[str]) -> int:
    ...
    for table in source_tables:
        try:
            with engine.begin() as conn:
                # Try with default schema first
                try:
                    result = conn.execute(text(f"SELECT MAX(block_number) FROM {table}"))
                except:
                    # Try with default. prefix if table name doesn't have schema
                    if '.' not in table:
...
def get_block_range(target_table: str, source_tables: List[str], source_engine, target_engine) -> Tuple[int, int]:
    """
    ...
    """
    start_block = get_max_block_from_table(target_table, target_engine)
    end_block = get_min_block_from_source_tables(source_engine, source_tables)
